...
const WORK_ADVANCEMENT = /核对|核验|整理|分析|评估|复盘|规划|筛查|筛选|验证|更新|汇总|测算|核算|估算|算一下|算出|预测|建模|生成|制作|输出|交付|判断|比较|补充|补齐|修订|导出|检查|排查|形成|列出|提取|拆分|对齐|标注|追溯|重算|排序|定位/iu;
...
const CALCULATION_COMPLEXITY = Object.freeze([
  /近\s*\d+\s*(?:个月|月|季度|年)|两期|本期.{0,8}(?:上年同期|同期)|(?:上年同期|同期).{0,8}本期|时间序列|同比|环比|趋势|滚动/iu,
