...
export function listDoubaoChatTargets(targets) {
  return targets.filter((target) => target?.type === "page"
    && /^(?:doubao|chrome):\/\/doubao-chat\/chat(?:[/?#]|$)/iu.test(String(target.url ?? "")));
}

export function selectDoubaoChatTarget(targets, { conversationId = "", targetId = "" } = {}) {
  const candidates = listDoubaoChatTargets(targets);
  if (targetId) {
    const matches = candidates.filter((target) => String(target.id) === String(targetId));
    if (matches.length !== 1) {
      throw new Error(`Expected Doubao target ${targetId}, found ${matches.length}.`);
    }
    return matches[0];
  }
  if (conversationId) {
    const matches = candidates.filter((target) => new RegExp(
      `/chat/${String(conversationId).replace(/[.*+?^${}()|[\]\\]/gu, "\\$&")}(?:[/?#]|$)`,
      "u",
    ).test(String(target.url ?? "")));
    if (matches.length !== 1) {
      throw new Error(`Expected one Doubao conversation ${conversationId}, found ${matches.length}.`);
    }
    return matches[0];
  }
  ...
  if (candidates.length !== 1) {
    throw new Error(`Expected one Doubao chat target, found ${candidates.length}; select one with targetId.`);
  }
  return candidates[0];
}
