...
export function isBlankOfficeTaskState(url, state) {
  return CHAT_URLS.has(String(url))
    && state?.officeModeActive === true
    && Number(state?.sentMessageCount ?? -1) === 0
    && Number(state?.receivedMessageCount ?? -1) === 0;
}
...
async function targetInfoForPage(page) {
...
export async function discoverDoubaoPages(browser) {
  const pages = browser.contexts().flatMap((context) => context.pages())
    .filter((page) => /^(?:doubao|chrome):\/\/doubao-chat\/chat(?:[/?#]|$)/iu.test(page.url()));
  return await Promise.all(pages.map(async (page) => {
    const targetInfo = await targetInfoForPage(page);
    return {
      browserContextId: String(targetInfo.browserContextId ?? ""),
      page,
...
export function selectDoubaoPageInfo(pageInfos, { conversationId = "", targetId = "" } = {}) {
  const candidates = Array.isArray(pageInfos) ? pageInfos : [];
  if (targetId) {
    const matches = candidates.filter((item) => item.targetId === String(targetId));
    if (matches.length !== 1) throw new Error(`Expected Doubao page target ${targetId}, found ${matches.length}.`);
    return matches[0];
  }
  if (conversationId) {
    const matches = candidates.filter((item) => conversationIdFromUrl(item.url) === String(conversationId));
    if (matches.length !== 1) throw new Error(`Expected Doubao conversation ${conversationId}, found ${matches.length}.`);
    return matches[0];
  }
  if (candidates.length !== 1) {
    throw new Error(`Expected one Doubao chat page, found ${candidates.length}; select one with targetId.`);
  }
  return candidates[0];
}

export async function connectDoubao(endpoint, selector = {}) {
  const browser = await chromium.connectOverCDP(endpoint);
  const pageInfos = await discoverDoubaoPages(browser);
  const pageInfo = selectDoubaoPageInfo(pageInfos, selector);
  return {
    browser,
    page: pageInfo.page,
    pageInfo: { ...pageInfo, page: undefined },
    pages: pageInfos.map((item) => ({ ...item, page: undefined })),
  };
}

export async function listDoubaoWindows(endpoint) {
  const browser = await chromium.connectOverCDP(endpoint);
  const pageInfos = await discoverDoubaoPages(browser);
  return pageInfos.map((item) => ({ ...item, page: undefined }));
}

export async function createDoubaoWindow(endpoint) {
  const browser = await chromium.connectOverCDP(endpoint);
  const existing = await discoverDoubaoPages(browser);
  if (!existing.length) throw new Error("Cannot create a Doubao window without an existing logged-in chat page.");
  ...
  try {
    ...
    if (existing[0].browserContextId) params.browserContextId = existing[0].browserContextId;
    created = await session.send("Target.createTarget", params);
  } finally {
    await session.detach().catch(() => {});
  }
  ...
  while (Date.now() < deadline) {
    const pageInfos = await discoverDoubaoPages(browser);
    const match = pageInfos.find((item) => item.targetId === created.targetId);
    if (match) return { ...match, page: undefined };
    await new Promise((resolve) => setTimeout(resolve, 100));
  }
  throw new Error(`Created Doubao target ${created.targetId} did not become a Playwright page.`);
}
...
export async function openNewOfficeTask(page) {
  const current = await inspectChat(page);
  const blankOfficeTask = isBlankOfficeTaskState(page.url(), current);
  if (blankOfficeTask) {
  ...
  try {
    ...
    if (!/outside of the viewport/iu.test(String(error?.message ?? "")) || !await button.isVisible()) throw error;
    // Doubao can keep this visible sidebar control in a transformed container that
    // Playwright cannot scroll into its calculated viewport. A DOM click preserves
    // the app's own handler while avoiding a second 30-second retry loop.
  ...
  while (Date.now() < deadline) {
    ...
    opened = await inspectChat(page);
    if (isBlankOfficeTaskState(page.url(), opened)) {
      await ensureLocalComputerDisabled(page);
      return await inspectChat(page);
    }
  }
  const error = new Error("A new blank Office Task did not replace the previous conversation; no prompt was sent.");
  error.code = "NEW_OFFICE_TASK_NOT_BLANK";
  error.previousUrl = current.url;
  error.currentUrl = opened?.url ?? page.url();
  throw error;
}
...
export async function dismissVisibleQuotaManagementDialog(page, { signal } = {}) {
  const quotaFrame = page.locator('iframe[title="订阅"][src*="/member/quota-management"]');
  let dismissedCount = 0;
  for (let attempt = 0; attempt < 3; attempt += 1) {
    throwIfJobPauseRequested(signal);
    const dialogs = page
      .locator('[role="dialog"][data-setting-modal-layer]:visible')
      .filter({ has: quotaFrame });
    const visibleCount = await dialogs.count();
    if (visibleCount === 0) {
      return { dismissed: dismissedCount > 0, dismissedCount };
    }
    await page.keyboard.press("Escape");
    dismissedCount += visibleCount;
  ...
  const remaining = await page
    ...
    .count();
  if (remaining > 0) {
    throw new Error("Quota-management dialog remained open after dismissal attempts.");
  }
  return { dismissed: dismissedCount > 0, dismissedCount };
}

async function clickMessageAction(page, action, { signal } = {}) {
  await dismissVisibleQuotaManagementDialog(page, { signal });
  throwIfJobPauseRequested(signal);
  try {
    await action.click({ timeout: 5_000 });
  } catch (error) {
    const recovery = await dismissVisibleQuotaManagementDialog(page, { signal });
    if (!recovery.dismissed) throw error;
    throwIfJobPauseRequested(signal);
...
export async function openLatestShare(page, { signal } = {}) {
  ...
  const panelAlreadyOpen = await title.count() === 1 && await copyButton.count() === 1;
  if (!panelAlreadyOpen) {
    const container = responses.nth(responseCount - 1);
    await container.hover({ force: true });
    ...
    await clickMessageAction(page, share, { signal });
    await title.first().waitFor({ state: "visible", timeout: 10_000 });
    await copyButton.first().waitFor({ state: "visible", timeout: 10_000 });
    if (await title.count() !== 1 || await copyButton.count() !== 1) {
      throw new Error("Share selection panel did not become uniquely available.");
    }
  }
...
export async function copyOpenShareLink(page, { selectAll = true, signal } = {}) {
  ...
  await page.keyboard.press("Escape");
  await title.first().waitFor({ state: "hidden", timeout: 5_000 });
  const panelClosed = await title.count() === 0 && await copyButton.count() === 0;
  if (!panelClosed) throw new Error("Share selection panel did not close after copying the link.");
  return {
    checkedAfter,
    checkedBefore,
    checkboxCount,
...
export async function openLatestMoreMenu(page) {
  ...
  await clickMessageAction(page, more);
  await page.waitForTimeout(500);
...
export async function copyLatestLogInfo(page, { signal } = {}) {
  ...
  const more = container.locator(SELECTORS.messageMore);
  if (await more.count() !== 1) throw new Error("A unique more-menu action was not available.");
  await clickMessageAction(page, more, { signal });
...
export async function openLatestFeedback(page, vote, { signal } = {}) {
  ...
  await clickMessageAction(page, action, { signal });
  await page.waitForTimeout(800);
...
export function responseWaitDeadline(timeoutMs, now = Date.now()) {
  const normalizedTimeoutMs = Number(timeoutMs);
  return Number.isFinite(normalizedTimeoutMs) && normalizedTimeoutMs > 0
...
export async function waitForResponse(page, {
  ...
  const normalizedTimeoutMs = Number(timeoutMs);
  const deadline = responseWaitDeadline(normalizedTimeoutMs);
  const responses = page.locator(SELECTORS.receivedMessage);
  let lastText = "";
  ...
  throw new Error(`Timed out after ${normalizedTimeoutMs}ms waiting for a complete response.`);
}
...
export async function recoverLatestSentExchange(page, text, {
  ...
  const error = new Error(`Timed out after ${normalizedTimeoutMs}ms recovering the visible Doubao response.`);
