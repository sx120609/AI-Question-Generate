...
const draft = JSON.parse(await fs.readFile(path.join(draftDir, "01_pre_de_ai.json"), "utf8"));
const deAi = JSON.parse(await fs.readFile(path.join(qaDir, "01_de_ai_rewrite.json"), "utf8"));
...
const factLedgerBuffer = await fs.readFile(factLedgerPath);
...
const finalRecord = {
  ...draft.sourceRecord,
  题目: deAi.rewrite.question,
};
const workflow = await loadProductionWorkflow(workflowPath);
recordDeAiRewrite(workflow, 1, deAi);
recordFinalRecord(workflow, 1, { recordUid: finalRecord.UID, finalRecord });
...
const sceneBundle = {
  ...
  schemaVersion: 1,
  factLedgerPath,
  factLedgerHash: sha256(factLedgerBuffer),
  cards: [{
...
const processReportPath = path.join(feishuDir, "production_trace_gate_report.json");
const processReceiptPath = path.join(feishuDir, "production_trace_gate_receipt.json");
const processResult = await runProductionTraceGate({
  packetPath: path.join(sourceDir, "production_input_packet.json"),
  tracePath,
  candidatePath,
  fillPlanPath,
  reportPath: processReportPath,
  receiptPath: processReceiptPath,
