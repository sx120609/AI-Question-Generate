...
const targeted = JSON.parse(await fs.readFile(path.join(qaDir, "01_de_ai_rewrite_targeted.json"), "utf8"));
...
let candidate = targeted.rewrite.question;
let selected = null;
const attempts = [];
for (let attempt = 1; attempt <= 3; attempt += 1) {
  const response = await rewriteMuguaDeAiText({
    ...
    promptText: [
      "你在修补一条已经通过相似度门禁的L1题面。输出完整题面JSON。",
      "只补回五处被遗漏的要求，同时压缩重复连接语，使全文保持在1160至1198个可见字符。",
      "开头明确产物是一份可下载的Excel成交复核表。",
      "附件段明确首轮一次性接收并核验十份附件。",
      "得分表段明确匿名编号保持原样，也不映射到供应商。",
      "抽检段明确建议只安排补证顺序，不形成验收或履约结论。",
      "来源段明确每个数值和判断均注明出处并可回到原件。",
      "所有现有数字、事实、七个分页和待补状态完整保留。其余句子尽量保持现状。",
      "每句话最多一个顿号。正文不使用分号、项目符号、编号、机械顺序词和客套请求。",
  ...
  attempts.push({ attempt, pass: validation.pass, visibleLength: validation.visibleLength, findings: validation.findings });
  await writeJson(path.join(qaDir, `01_de_ai_post_preflight_repair_attempt_${attempt}.json`), { response, rewrite, validation });
  if (validation.pass) {
    selected = { attempt, response, rewrite, validation };
    break;
  }
}
if (!selected) throw new Error(`Post-preflight de-AI repair failed: ${JSON.stringify(attempts)}`);
...
const deAi = {
  ...
  attempts: attempts.map((item) => ({
    ...
    pass: item.pass,
    visibleLength: item.visibleLength,
    findingRules: item.findings.map((finding) => finding.rule),
  })),
  rewrite: selected.rewrite,
  validation: selected.validation,
};
await writeJson(path.join(qaDir, "01_de_ai_rewrite_final.json"), deAi);

const postDeAiGate = await localJson({
  label: "06_post_de_ai_preflight_final",
  systemPrompt: [
    "你是发送给豆包之前的独立可见文本审查员。只审查，不改写，不回答题目。",
    "核对可下载产物、首轮一次性接收十份附件、535条记录、5889台、六包数字、金额冲突、质保比例、字段异常、匿名编号不映射、待补边界、七个表页和两包抽检请求。",
    "确认每个数值和判断都有回溯要求。确认抽检建议不形成验收或履约结论。",
    "拦截客套请求、边界式教唆表达、分号、每句超过一个顿号、内部错误信息、工具痕迹和奇怪标点。发现任何问题都返回pass=false。",
  ].join("\n"),
  userPrompt: JSON.stringify({ sourceQuestion, outboundQuestion: deAi.rewrite.question, attachmentNames: manifest.items.map((item) => item.name) }, null, 2),
  outputSchema: object({
    pass: { type: "boolean" },
...
if (!postDeAiGate.pass || postDeAiGate.issues.length || !postDeAiGate.factsPreserved
  ...
  throw new Error(`Final post-de-AI preflight blocked: ${JSON.stringify(postDeAiGate)}`);
}
