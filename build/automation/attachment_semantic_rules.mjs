...
export function evaluateAttachmentSemantics(plan = {}, { allowPreservedLegacy = false } = {}) {
  ...
  if (!attachments.length) findings.push({ rule: "attachment-set-empty" });
  if (specificShare < MIN_SPECIFIC_BUSINESS_SHARE) {
    findings.push({
      rule: "specific-business-share-below-minimum",
      expectedMinimum: MIN_SPECIFIC_BUSINESS_SHARE,
