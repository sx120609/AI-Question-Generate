...
function interactionRewriteMessages({ job, prompt, recentPrompts, roundNumber, textPurpose, validationFeedback }) {
  ...
  const interactionVoiceRule = textPurpose === "chat-prompt"
    ? "按上下文自然决定是否使用“你”或“我”。需要承接责任人、上一轮体验或直接委托时可以使用；材料介绍、事实说明和直接任务句无需强塞代词。不得增加“我这边”“售后这边”“团队这边”一类角色填充语。"
...
export function buildEvaluatorMessages({
  ...
  const system = `你是独立的豆包办公任务轨迹评审与追问规划器。\n`
    ...
    + `taskOutcome 为 continue 时，nextPrompt 必须像真人解决问题：先承接本轮一个具体结论、数字、引用、口径、遗漏或产物状态，再只向下一层收窄。每次只给一个主诉求，并可带一个直接支撑它的子诉求。主体至少推进一个实质判断，例如核对事实与来源、解释差异、处理证据冲突、复核计算口径或决定取舍。导出、下载、改格式、增删列、重命名、检查能否打开和保持可编辑只能作为直接相关的附带交付动作，不能单独占一轮，也不能为了满足最低轮数制造操作。不能重复整道题、机械说“继续完善”、一次塞入多个方向，也不能提前穷举未来风险、回滚分支和限制清单。不能提到轮次、评审器、模型或自动化。\n`
    + `nextPrompt 使用正向动作句，直接说明本次要核对、拆分、补齐或生成的内容和验收状态。省去否定式边界提醒、对立句式和教唆式口吻。\n`
    + `nextPrompt 按上下文自然决定是否使用“你”或“我”。材料说明和直接任务句可以省略代词，不得为了凑人称增加无信息量开场。全文禁用分号，每句话最多一个顿号。\n`
