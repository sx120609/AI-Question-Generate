...
export async function runDoubaoJob({
  ...
  if (resume) {
    ...
    if (isResumablePromptQualityRound(nextRound, state.error)) {
  ...
  try {
    ...
    for (let index = state.rounds.length; index < config.maxRounds; index += 1) {
      ...
      const repeatedQuotaRoundsBeforePlanning = quotaUnavailableRoundCount(state);
      ...
      decision ??= (config.mode === "scripted"
        ...
        : await requestPolicyDecision({
          ...
          decisionValidator: index + 1 < config.maxRounds && repeatedQuotaRoundsBeforePlanning < 3
