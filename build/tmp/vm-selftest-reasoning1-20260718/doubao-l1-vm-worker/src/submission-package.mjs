...
export function buildIncompleteSubmissionPackage(state, { target = null } = {}) {
  ...
  if (!Array.isArray(state?.rounds) || state.rounds.length < minimumRounds
    ...
    throw new Error("Doubao-gap submission package requires the configured minimum completed rounds.");
  }
  if (state?.finalProductAcceptance?.accepted !== false) {
    throw new Error("Doubao-gap submission package requires a rejected final product assessment.");
  }
  if (!['doubao-unable', 'hard-limit-reached'].includes(String(state?.completionOutcome ?? ""))
    || !Array.isArray(state?.unresolvedIssues) || state.unresolvedIssues.length === 0) {
    throw new Error("Doubao-gap submission package requires a terminal outcome and documented unresolved issues.");
  }
  return {
    schemaVersion: 3,
    kind: "doubao-feishu-submission-package",
    ...
    unresolvedIssues: structuredClone(state.unresolvedIssues),
    roundCount: state.rounds.length,
    productAcceptance: structuredClone(state.finalProductAcceptance),
    conversation: {
      conversationId: state.conversationId,
...
export async function writeIncompleteSubmissionPackage(state, { resultPath, target = null } = {}) {
  ...
  try {
    ...
    const finalBytes = await readFile(packagePath);
    const parsed = JSON.parse(finalBytes.toString("utf8"));
    if (parsed.status !== "READY_WITH_DOUBAO_GAP" || parsed.roundCount !== state.rounds.length) {
      throw new Error("Doubao-gap submission package final readback failed structural verification.");
    }
    return {
      artifactPath: path.relative(path.dirname(resultPath), packagePath).replace(/\\/gu, "/"),
      pass: true,
