...
生产固定六轮。每轮消息和人评先经 Mugua Gemini 3.1 Pro 去AI，再由本机已登录的 Codex `gpt-5.6-sol` 规划或质检。任何模型失败都会按限定次数重试，耗尽后暂停，不会 fallback 放行。任务完成后只生成飞书 `READY_NOT_SUBMITTED` 包，不会自动提交飞书。
