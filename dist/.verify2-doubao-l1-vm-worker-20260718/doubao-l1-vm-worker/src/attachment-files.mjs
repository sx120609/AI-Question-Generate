...
import { evaluateAttachmentSemantics } from "../../build/automation/attachment_semantic_rules.mjs";
...
export function validateAttachmentConfig(value) {
  ...
  for (const [index, attachment] of value.attachments.entries()) {
    ...
    if (!/^https?:\/\//iu.test(String(attachment?.sourceUrl ?? "").trim())) {
      throw new Error(`${label}.sourceUrl must be an HTTP(S) source URL.`);
    }
  }
