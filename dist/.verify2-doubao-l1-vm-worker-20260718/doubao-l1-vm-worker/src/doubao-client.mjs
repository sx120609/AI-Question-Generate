...
export function deriveLocalComputerState(buttons = []) {
  ...
  const explicitActive = checked === "true"
    || pressed === "true"
    || selected === "true"
    || expanded === "true"
    || state === "open";
...
async function readComposer({ kind, locator }) {
  ...
  return await locator.evaluate((element) => element.innerText.replace(/\n$/u, ""));
}
  ...
  return {
    ...
    composerPlaceholder: standardComposerCount === 1
      ...
      : officeComposerCount === 1
        ? await officeComposer.locator("p").getAttribute("data-placeholder")
        : null,
...
export async function uploadComposerAttachments(page, attachments, {
  ...
  if (multiple) {
    ...
    for (let index = 0; index < absolutePaths.length; index += 1) {
      throwIfJobPauseRequested(signal);
      await input.setInputFiles(absolutePaths[index]);
...
async function responseArtifactEvidenceFromContainer(container) {
  return await container.evaluate((element) => {
    ...
    for (const candidate of element.querySelectorAll(selectors)) {
      ...
      const key = JSON.stringify([text, href, label, dataTestId]);
      if (!text && !href && !label) continue;
      if (seen.has(key)) continue;
      seen.add(key);
      result.push({ dataTestId, href, label, text });
...
async function responseIdentityFromContainer(container) {
  return await container.evaluate((element) =>
    element.closest("[data-observe-row]")?.getAttribute("data-observe-row")
      || element.closest('[data-testid="union_message"]')?.getAttribute("data-target-id")
      || "");
}
...
export async function copyOpenShareLink(page, { selectAll = true, signal } = {}) {
  ...
  if (selectAll && checkedBefore.some((checked) => !checked)) {
  ...
  if (selectAll && checkedAfter.some((checked) => !checked)) {
    throw new Error("Not every visible share checkbox was selected.");
  }
...
export async function waitForResponse(page, {
  ...
  while (Date.now() < deadline) {
    ...
    if (count > 0) {
      ...
      const identity = await responseIdentityFromContainer(container);
      const isNewResponse = count > previousCount || (identity && identity !== previousIdentity);
      if (!isNewResponse) {
        await page.waitForTimeout(pollMs);
        continue;
      }
...
export async function sendAndWait(page, text, options = {}) {
  ...
  const previousIdentity = previousCount > 0
    ? await responseIdentityFromContainer(responses.nth(previousCount - 1))
    : "";
  if (previousCount > 0 && !previousIdentity) {
    throw new Error("A stable identity was not available for the latest response.");
  }
  const startedAt = new Date().toISOString();
  ...
  let response;
  try {
    response = await waitForResponse(page, { previousCount, previousIdentity, signal, ...waitOptions });
  } catch (error) {
