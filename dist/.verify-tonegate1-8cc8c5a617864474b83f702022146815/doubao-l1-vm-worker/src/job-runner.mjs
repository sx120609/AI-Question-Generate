...
export async function runDoubaoJob({
  ...
  if (resume) {
    ...
    if (nextRound?.status === "sending" && nextRound.prompt && !nextRound.response) {
      resumableSendingRound = nextRound;
    } else if (["response_received", "capturing_product_screenshot", "response_visibility_pass"].includes(nextRound?.status)
      && nextRound.response?.response && nextRound.response?.responseIdentity) {
  ...
  try {
    ...
    for (let index = state.rounds.length; index < config.maxRounds; index += 1) {
      ...
      if (round.responseScreenshot?.pass) {
        if (round.responseScreenshot.responseIdentity !== round.response.responseIdentity
          || Number(round.responseScreenshot.responseCount) !== Number(round.response.responseCount)) {
          throw new Error("Existing response screenshot does not match the resumable response.");
        }
      } else {
        ...
        const screenshot = await captureLatestResponseScreenshot(page, {
          expectedCount: round.response.responseCount,
          expectedIdentity: round.response.responseIdentity,
          expectedText: round.response.response,
