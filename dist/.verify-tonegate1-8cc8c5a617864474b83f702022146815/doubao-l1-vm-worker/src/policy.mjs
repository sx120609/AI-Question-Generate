...
export function auditOutboundPrompt(value, { recentPrompts = [] } = {}) {
...
function interactionRewriteMessages({ job, prompt, recentPrompts, roundNumber, textPurpose }) {
...
export async function requestInteractionRewrite({
  ...
  const completion = await runModelWithRetry(
    () => requestModelPayload({
      ...
      body: {
        ...
        messages: interactionRewriteMessages({
          job,
          prompt: sourcePrompt,
          recentPrompts,
          roundNumber,
  ...
  try {
    ...
    rewrittenPrompt = validateOutboundPrompt(parsed?.prompt, { recentPrompts });
...
function parsePromptPreflight(text, prompt, { recentPrompts = [] } = {}) {
  const parsed = parseDecisionJson(text);
  const checkedPrompt = validateOutboundPrompt(prompt, { recentPrompts });
...
export async function requestPromptPreflight({
  ...
  let checked;
  try {
    checked = parsePromptPreflight(payload?.content, originalPrompt, { recentPrompts });
  } catch (error) {
