...
- 将飞书表要求的六轮 Prompt、人评、产物验收、全选分享链接和日志 ID 编译为 `READY_NOT_SUBMITTED` 待提交包；
...
等对立句式会被确定性门禁拦截。
每轮任务消息至少自然出现一次“你”或“我”。全文禁用分号，每句话最多一个顿号。
...
`conversationId`、六轮 prompt/response、逐轮评价和附件上传回执。
同名 `<result>.artifacts/` 目录只保存 `feishu-submission-package.json`。
